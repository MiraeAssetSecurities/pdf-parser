...
PDF 파서는 두 가지 OCR 엔진을 지원합니다. 문서 유형과 요구사항에 따라 적합한 엔진을 선택할 수 있습니다.

### 성능 비교 (4페이지 PDF 기준)

| 항목 | Docling | LayoutPredictor |
|------|---------|-----|
| 처리 시간 (초) | 57.3 | 6.14 |
| 페이지 수 | 4 | 4 |
| 감지된 그림 수 | 1 | 24 |
...
| 항목 | Docling (기본) | LayoutPredictor |
|------|---------------|-------------------|
| 속도 | 느림 (TableFormer 모델 포함) | **약 9배 빠름** |
| 그림 감지 | 보수적 (정확도 높음) | **적극적** (더 많은 요소 감지) |
| 테이블 감지 | 보수적 (정확도 높음) | **적극적** (더 많은 테이블 감지) |
| 테이블 구조 분석 | TableFormer로 셀 구조 정밀 분석 | 레이아웃 기반 분석 |
| 이미지 분류 | EfficientNet-B0 (16 카테고리) | EfficientNet-B0 (16 카테고리) |
| 적합한 용도 | 정밀한 테이블 구조가 필요한 문서 | 빠른 처리가 필요한 대량 문서 |
...
curl -X POST http://localhost:3000/ocr \
  ...
  -d '{"inputPath": "s3://bucket/file.pdf", "outputPath": "s3://bucket/output/"}'

# IBM OCR
curl -X POST http://localhost:3000/ocr \
  -H "Content-Type: application/json" \
  -d '{"inputPath": "s3://bucket/file.pdf", "outputPath": "s3://bucket/output/", "ocrMode": "ibm"}'
```
...
│   ├── s3_browser.py             # S3Browser (JupyterLab 인터랙티브 위젯)
│   ├── table_merger.py           # 테이블 병합 유틸리티
...
**API 엔드포인트:**
- `POST /process`: PDF 전체 파이프라인 (OCR + LLM 요약 + 마크다운)
- `POST /ocr`: OCR 전용 처리 (바운딩 박스 시각화 포함, `docling` / `ibm` 선택 가능)
- `POST /process-office`: Office 문서 처리
- `POST /process-document`: PDF/Office 파일 확장자 자동 감지 통합 엔드포인트
- `GET /health`: 서버 상태 확인
...
> 서버 배포 방법(systemd 서비스)은 [setting.md](setting.md)를 참조하세요.
...
config = OfficeParserConfig(
    summarize=True,
    bedrock_model_id="ap-northeast-2.anthropic.claude-haiku-4-5-20251001-v1:0",
)
...
| `--bedrock-region` | `ap-northeast-2` | Bedrock 리전 |
...
| 형식 | 확장자 | 파서 |
|------|--------|------|
| PDF | `.pdf` | `pdf_parser` (Docling / IBM) |
...
```
output/{파일명}/
├── {파일명}_final.md             # 최종 마크다운 (HTML 메타데이터 포함, PDF만)
├── {파일명}_text.md              # 원본 텍스트 마크다운
├── pictures/                     # 추출된 이미지
│   └── {category}/
└── table/                        # (PDF만) 테이블 에셋
    ├── img/                      # 테이블 영역 이미지
